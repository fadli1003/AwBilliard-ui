...
const SignUp = () => {
	...
	return (
		<div className='min-h-screen w-full flex justify-center items-center'>
			<Title title='Sign Up' />
			{errors?.msg && (
				<span className='absolute top-15 right-5 text-red-500 bg-white/20 rounded-full text-sm px-4.5 py-1.5'>
					{errors.msg}
				</span>
			)}
			<form
			...
			>
				...
				<div className='flex flex-col gap-2 relative'>
					...
					/>
					{errors?.name && (
						<span className='absolute -bottom-5 text-red-500 text-xs'>{errors.name}</span>
					)}
				</div>
				<div className='flex flex-col gap-2 relative'>
					...
					/>
					{errors?.email && (
						<span className='absolute -bottom-5 text-red-500 text-xs'>{errors.email}</span>
					)}
				</div>
				<div className='flex flex-col gap-2 relative'>
					...
					/>
					{errors?.phone && (
						<span className='absolute -bottom-5 text-red-500 text-xs'>{errors.phone}</span>
					)}
				</div>
				<div className='flex flex-col gap-2 relative'>
				...
				<button
					disabled={loading}
					type='submit'
					className='font-semibold border rounded-full py-1.75 mt-2 flex gap-2 justify-center items-center cursor-pointer hover:bg-white/5'
				>
					{loading ? (
						<>
							<Loader2
